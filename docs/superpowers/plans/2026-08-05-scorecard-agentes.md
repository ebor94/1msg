...
- **Agentes desactivados con chats abiertos**: tanto el backlog en vivo como la tabla del día mapean solo sobre `wa_agentes WHERE activo=1`. Si un agente se desactiva mientras aún tiene chats sin responder, esos chats no aparecen en el backlog hasta que un mensaje nuevo los reasigne, y su volumen histórico desaparece de la tabla. Es el mismo comportamiento sistémico de la desactivación (los chats abiertos quedan huérfanos hasta el próximo mensaje); se aborda en el flujo de desactivación, no aquí.
- **`totales.chatsAtendidos`** suma el `COUNT(DISTINCT conversacion_id)` de cada agente, así que una conversación tocada por dos agentes (handoff) se cuenta una vez por agente e infla ese total. Los valores por agente son correctos; solo el total puede exceder el nº real de chats distintos.
- **Reacciones/mensajes de sistema**: se persisten como filas `in`/`out` reales, así que una reacción 👍 del cliente marca `ultimo_mensaje_dir='in'` (puede mostrar como "sin responder" un chat que el cliente solo reaccionó) y puede ser el ancla de un turno de TPR. Impacto acotado (los agentes no envían reacciones por este backend). Filtrar `tipo IN ('reaction','system')` queda para una iteración.
- **`NOW()` en el backlog** asume que la zona de sesión de MySQL es la de Colombia (mismo supuesto que los `NOW()` ya existentes al escribir `tomada_en`). La verificación en vivo debe confirmar que un tiempo de espera cuadra con la realidad.
